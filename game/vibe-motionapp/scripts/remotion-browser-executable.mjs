...
const runRemotionBrowserEnsure = ({ log }) => {
  log(`Cache miss. Running \`/usr/local/Cellar/node/25.8.2/bin/pnpm exec remotion browser ensure\` in project context.`);
  const result = spawnSync("/usr/local/Cellar/node/25.8.2/bin/pnpm", ["exec", "remotion", "browser", "ensure"], {
    stdio: "inherit",
    env: process.env,
  });
