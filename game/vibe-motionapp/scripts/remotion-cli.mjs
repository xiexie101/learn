...
const run = async () => {
  ...
  const commandForLog = ["/usr/local/Cellar/node/25.8.2/bin/pnpm", "exec", ...remotionArgs].join(" ");
  console.log(`[Remotion] running ${commandForLog}`);

  const result = spawnSync("/usr/local/Cellar/node/25.8.2/bin/pnpm", ["exec", ...remotionArgs], {
    stdio: "inherit",
    env,
  });
