...
date: 2019-01-29
url: https://sorrycc.com/polyfill
---
...
![](https://img.alicdn.com/imgextra/i1/O1CN01XXOppo1hEGDB2T98T_!!6000000004245-2-tps-1202-308.png)
...
![](https://img.alicdn.com/imgextra/i3/O1CN01ADh28H1LHztgkChB9_!!6000000001275-2-tps-1002-306.png)
...
### 在线补丁，比如：[polyfill.io](http://polyfill.io)

前面的手动引入解决的是特性列表的问题，有了特性列表，要做到按需下载，就需要用到在线的补丁服务了。目前最流行的应该就是 [polyfill.io](https://polyfill.io/)，提供的是 cdn 服务，有些站点在用，例如 [https://spectrum.chat/](https://spectrum.chat/)。另外，[polyfill.io](http://polyfill.io) 还开源了 [polyfill-service](https://github.com/financial-times/polyfill-service) 供我们自己搭建使用。
...
按需特性探测可以用 `@babel/preset-env` 配上 `targets` 以及试验阶段的 `useBuiltIns: usage`，保障特性集的最小化。之所以说是未来，因为 JavaScript 的动态性，语法探测不太可能探测出所有特性，但上了 TypeScript 之后可能会好一些。另外，要注意一个前提是 node\_modules 也需要走 babel 编译，不然 node\_modules 下用到的特性会探测不出来。

在线补丁可以用类似前面介绍的 [https://polyfill.io/](https://polyfill.io/) 提供的方案，让浏览器只下载必要的补丁，通常大公司用的话会部署一份到自己的 cdn 上。（阿里好像有团队部署了，但一时间想不起地址了。）
...
目前推荐是用 `core-js/modules`，因为 node\_modules 不走 babel 编译，所以 `core-js/library` 的方式无法为依赖库提供补丁。
...
*   [http://2ality.com/2011/12/shim-vs-polyfill.html](http://2ality.com/2011/12/shim-vs-polyfill.html)
*   [https://babeljs.io/docs/en/next/babel-preset-env.html](https://babeljs.io/docs/en/next/babel-preset-env.html)
*   [https://github.com/zloirock/core-js/tree/v2](https://github.com/zloirock/core-js/tree/v2)
